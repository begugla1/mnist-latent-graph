...
class LatentGraphRecovery:
    ...
    def fit(self, X: np.ndarray, X_val: np.ndarray | None = None):
        ...
        # All-pairs correlation screening. Full Gram matrices are also built below.
        ...
        for i, ci in enumerate(candidates):
            ...
            for alpha in cfg.lasso_alphas:
                model = Lasso(
                    alpha=alpha, fit_intercept=False, max_iter=3000, selection="cyclic"
                ).fit(A, y)
        ...
        def local_score(i, neigh):
            if not neigh:
                return float(np.log(max(self._C[i, i], 1e-12)))
        ...
        edges = set(starts[selected_start])
        ns = neighbors(edges)
        ...
        T0 = max(float(np.median(probe_d)), 1e-3)
        Tf = T0 * 1e-3
        for step in range(cfg.anneal_steps):
            ...
            for _ in range(cfg.proposal_batch):
                if edges and rng.random() < 0.55:
                    ...
                    if all_pairs is None:
                        all_pairs = tuple(combinations(range(p), 2))
                    for _ in range(20):
                    ...
                    proposals.append(e)
            ranked = []
            history_weight = np.clip((step / cfg.anneal_steps - 0.10) / 0.20, 0, 1)
            for e in proposals:
                d = delta(e)
                unique_tested.add(e)
                h = history.get(e, (d, d * d, np.sign(d)))
                uncertainty = max(h[1] - h[0] ** 2, 0) ** 0.5
                rank = d + history_weight * (0.15 * uncertainty - 0.10 * abs(h[2]))
            ...
            history[e] = (
                beta * oldm + (1 - beta) * d,
                beta * oldq + (1 - beta) * d * d,
                beta * olds + (1 - beta) * np.sign(d),
            )

        # Return to the best visited state and try up to six deletion-pruning passes.
        # Reaching the pass limit does not certify a deletion-local minimum.
        after_annealing_best_edges = set(best_edges)
        edges = set(best_edges)
        ns = neighbors(edges)
        current = best_energy
        post_pruned = 0
        for _ in range(6):
        ...
        self.edges_ = (
            self.active_[self.edges_active_]
            if len(best_edges)
        ...
        self.stage_transitions_ = {
            "start_to_initial_pruning": transition(
                selected_start_edges, after_initial_pruning_edges
            ),
            "initial_pruning_to_annealing_best": transition(
                after_initial_pruning_edges, after_annealing_best_edges
            ),
            "annealing_best_to_final_pruning": transition(
                after_annealing_best_edges, best_edges
            ),
...
def recover_graph(
    X: np.ndarray, X_val: np.ndarray | None = None, config: RecoveryConfig | None = None
):
